        ...
        (function (index) {
            squares[i].onclick = function () {
                if (squares[index + 7].classList.contains('taken') && !squares[index].classList.contains('taken')) {
                    //add player's class
                    squares[index].classList.add(currentPlayer === 1 ? 'player-one' : 'player-two')
                    squares[index].classList.add('taken')
                    //change player
                    currentPlayer = currentPlayer === 1 ? 2 : 1;
                    currentPlayerText.innerHTML = currentPlayer === 1 ? 'Reimu\'s Turn!' : 'Marisa\'s Turn!';
                }
            }
        })(i)
    ...
    function checkBoard() {
        ...
        for (let y = 0; y < winningArrays.length; y++) {
            ...
            if (square1.classList.contains('player-one') &&
                ...
                currentPlayerText.className = 'h1'
                currentPlayerText.innerHTML = 'Reimu wins!'
                ...
                currentPlayerText.className = 'h1'
                currentPlayerText.innerHTML = 'Marisa wins!'
                squares.forEach(s => s.onclick = 'null')
            }
        }
    }
